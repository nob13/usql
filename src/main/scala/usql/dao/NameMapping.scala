...
trait NameMapping {
  ...
  def caseClassToTableName(name: String): SqlColumnId
}
...
object NameMapping {
  ...
  object Default extends NameMapping {
    ...
    override def caseClassToTableName(name: String): SqlColumnId = {
      SqlColumnId.fromString(snakeCase(getSimpleClassName(name)))
    }
  }
